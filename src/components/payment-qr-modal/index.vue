  ...
  >
    <div class="pay-qr-wrap">
      ...
      <a-space class="pay-actions" wrap>
        <a-button
          v-if="isSandbox && canSandboxPay"
          type="primary"
          :loading="sandboxLoading"
          @click="handleSandboxPay"
        >
          模拟支付成功
        </a-button>
...
<script lang="ts" setup>
  ...
  const canSandboxPay = computed(() =>
    hasPermission(Permissions.Payment.Order.SandboxPay)
  );

  const SANDBOX_CHANNEL = 'sandbox_native';
  ...
  const isSandbox = computed(() => channelCode.value === SANDBOX_CHANNEL);
  ...
  async function applyOrder(order: PaymentOrderDto, url?: string) {
    ...
    channelCode.value = order.channelCode;
  ...
  async function handleSandboxPay() {
    ...
    try {
      const { data } = await sandboxPayOrder(orderNo.value);
      status.value = data.status;
