<template>
  <section class="form-make-box">
          ...
          >
            <template #item="{ element }">
              <div class="field-box">
                ...
                <div
                  v-if="element.icon.indexOf('widget') == -1"
                  class="field-icon iconfont-approval-admin"
                  v-html="element.icon"
                ></div>
            ...
            <template #item="{ element }">
              <div
                :class="[
                  ...
                  formulaErrorWidgetNames.includes(element.name)
                    ? 'field-widget-error'
                    : '',
                ]"
              ...
              >
                <template v-if="element.focus">
                  ...
                  <div
                    class="widget-delete"
                    @click.stop.prevent="handleWidgetDelete(element)"
                    ><icon-close :size="12"
                  /></div>
                </template>
                <template
                ...
                >
                  <div
                    :class="['form-item', element.required ? 'required' : '']"
                  >
    ...
    >
      <template #title>
        {{ (fieldSetting[widget.type] || {}).label || '' }}
      </template>
      <div>
        <a-form :model="widget" layout="vertical">
          ...
          <a-form-item
            field="placeholder"
            label="提示"
            v-if="![WIDGET.DETAIL, WIDGET.FORMULA].includes(widget.type)"
          >
            <a-input
              v-if="![WIDGET.DESCRIBE].includes(widget.type)"
              v-model="widget.placeholder"
              :max-length="16"
            />
          ...
          <a-form-item
            field="unit"
            label="单位"
            v-if="[WIDGET.NUMBER].includes(widget.type)"
          >
          ...
          <a-form-item
            ...
            label="选项"
            v-if="
              [WIDGET.SINGLE_CHOICE, WIDGET.MULTI_CHOICE].includes(widget.type)
            "
          >
            <div class="select-options-box">
              <div class="option-item" v-for="(option, idx) in widget.options">
                <div class="option"
                  ><a-input
                    v-model:model-value="widget.options[idx]"
                    :max-length="16"
                /></div>
                <div
                  class="delete"
                  v-if="widget.options.length > 1"
                  @click="handleDeleteWidgetOption(widget, idx)"
                >
                  <icon-close :size="16" />
                </div>
              </div>
          ...
          <a-form-item label="格式" v-if="[WIDGET.MONEY].includes(widget.type)">
            <a-grid :cols="1">
              ...
              <a-grid-item
                ><a-checkbox v-model:model-value="widget.comma"
                  >显示千位分隔符</a-checkbox
                ></a-grid-item
              >
            </a-grid>
          ...
          <a-form-item
            label="日期类型"
            v-if="[WIDGET.DATE, WIDGET.DATE_RANGE].includes(widget.type)"
          >
            <a-radio-group
              direction="vertical"
              v-model:model-value="widget.format"
            >
          ...
          <a-form-item
            label="其他选项"
            v-if="
              ![WIDGET.DESCRIBE, WIDGET.DETAIL, WIDGET.FORMULA].includes(
                widget.type
              )
            "
          >
            <a-grid :cols="1">
              <a-grid-item>
                <a-checkbox v-model:model-value="widget.required"
                  >必填</a-checkbox
                >
              </a-grid-item>
              <a-grid-item
                v-if="
                  ![
                    ...
                    WIDGET.RICH_TEXT,
                    WIDGET.RATE,
                  ].includes(widget.type) && isNotInDetailWidget(widget)
                "
              >
                <a-checkbox v-model:model-value="widget.summary"
                  >显示为摘要</a-checkbox
                >
              </a-grid-item>
            </a-grid>
          ...
          >
            <div class="formula-box">
              <div class="formula-text" v-if="widget.formulaItems">{{
                widget.formulaItems.map((item) => item.name).join('')
              }}</div>
              <a-link class="formula-btn" @click="handleFormulaOpen">
                设置公式
                <template #icon>
                  <svg-icon icon-class="btn-formula" />
                </template>
              </a-link>
            </div>
          </a-form-item>
        ...
        <!-- 计算公式 -->
        <Formula
          v-model:visible="isFormulaOpened"
          :widgets="formulaWidgets"
          :formula-items="widget.formulaItems"
          @ok="handleFormulaOk"
        />
      </div>
    </a-drawer>
...
<script setup>
  ...
  let fieldText = ref([
    {
      type: WIDGET.SINGLELINE_TEXT,
      label: fieldSetting[WIDGET.SINGLELINE_TEXT].label,
      placeholder: '请输入',
      icon: '',
    },
    {
      type: WIDGET.MULTILINE_TEXT,
      label: fieldSetting[WIDGET.MULTILINE_TEXT].label,
      placeholder: '请输入',
      icon: '&#xe616',
    },
    {
      type: WIDGET.RICH_TEXT,
      label: fieldSetting[WIDGET.RICH_TEXT].label,
      placeholder: '请输入',
      icon: 'widget-rich-text',
    },
    {
      type: WIDGET.DESCRIBE,
      label: fieldSetting[WIDGET.DESCRIBE].label,
      placeholder: '说明',
      icon: '&#xe612',
    },
  ]);
  let fieldNum = ref([
    {
      type: WIDGET.NUMBER,
      label: fieldSetting[WIDGET.NUMBER].label,
      placeholder: '请输入数字',
      icon: '&#xe625',
    },
    {
      type: WIDGET.MONEY,
      label: fieldSetting[WIDGET.MONEY].label,
      placeholder: '请输入金额',
      icon: '&#xe60b',
    },
  ]);
  let fieldSelect = ref([
    {
      type: WIDGET.SINGLE_CHOICE,
      label: fieldSetting[WIDGET.SINGLE_CHOICE].label,
      placeholder: '请选择',
      icon: '&#xe637',
    },
    {
      type: WIDGET.MULTI_CHOICE,
      label: fieldSetting[WIDGET.MULTI_CHOICE].label,
      placeholder: '请选择',
      icon: '&#xe621',
    },
  ]);
  let fieldDate = ref([
    {
      type: WIDGET.DATE,
      label: fieldSetting[WIDGET.DATE].label,
      placeholder: '请选择日期',
      icon: '&#xe60c',
    },
    {
      type: WIDGET.DATE_RANGE,
      label: fieldSetting[WIDGET.DATE_RANGE].label,
      placeholder: '请选择日期区间',
      icon: '&#xe610',
    },
  ]);
  let fieldOther = ref([
    {
      type: WIDGET.DETAIL,
      label: fieldSetting[WIDGET.DETAIL].label,
      icon: 'widget-detail',
    },
    {
      type: WIDGET.PICTURE,
      label: fieldSetting[WIDGET.PICTURE].label,
      icon: '&#xe611',
    },
    {
      type: WIDGET.ATTACHMENT,
      label: fieldSetting[WIDGET.ATTACHMENT].label,
      icon: 'widget-attachment',
    },
    {
      type: WIDGET.DEPARTMENT,
      label: fieldSetting[WIDGET.DEPARTMENT].label,
      placeholder: '请选择部门',
      icon: 'widget-deptment',
    },
    {
      type: WIDGET.EMPLOYEE,
      label: fieldSetting[WIDGET.EMPLOYEE].label,
      placeholder: '请选择员工',
      icon: 'widget-employee',
    },
    {
      type: WIDGET.FLOW_INST,
      label: fieldSetting[WIDGET.FLOW_INST].label,
      placeholder: '请选择审批',
      icon: '&#xe61a',
    },
    {
      type: WIDGET.AREA,
      label: fieldSetting[WIDGET.AREA].label,
      placeholder: '请选择区域',
      icon: 'widget-city',
    },
    ...
    {
      type: WIDGET.RATE,
      label: fieldSetting[WIDGET.RATE].label,
      placeholder: '请输入评分',
      icon: 'widget-rate',
    },
    {
      type: WIDGET.FORMULA,
      label: fieldSetting[WIDGET.FORMULA].label,
      icon: 'widget-formula',
    },
  ]);
  let fields = ref([
    ...
    if (level1WidgetNames.includes(name)) {
      if (type == WIDGET.DETAIL) {
        ...
        formulaWidgets.value = (widgets.value || []).filter((i) => {
          let { required, type } = i;
          return (
            (required && [WIDGET.NUMBER, WIDGET.MONEY].includes(type)) ||
            type == WIDGET.DETAIL
          );
        });
      }
      ...
      if (type == WIDGET.FORMULA) {
        for (let detailWidget of detailWidgets) {
          ...
          let detailWidgetNames = (details || []).map(
            (detailWidget) => detailWidget.name
          );
          if (detailWidgetNames.includes(name)) {
            formulaWidgets.value = (details || []).filter((i) => {
              return (
                i.required && [WIDGET.NUMBER, WIDGET.MONEY].includes(i.type)
              );
            });
            break;
          }
  ...
  const handleFormulaOk = (formulaItems) => {
    if (formulaItems && formulaItems.length) {
      ...
      if (formulaErrorWidgetNames.value) {
        formulaErrorWidgetNames.value = formulaErrorWidgetNames.value.filter(
          (i) => i != widget.value.name
        );
      }
    } else {
  ...
  onMounted(() => {
    ...
    widgets.value.forEach((i) => {
      ...
      if ([WIDGET.DETAIL].includes(i.type))
        i.details.forEach((ii) => ii.focus && handleWidgetClick(i));
    });
  });
