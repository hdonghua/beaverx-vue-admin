      ...
      >
        ...
        <a-form-item field="flowAdminIds" label="流程管理员" required>
          <div class="flow-admin-box">
            <a-tag size="large" v-for="item in flowDef.flowAdminIds">{{
              getUserById(item).name
            }}</a-tag>
          </div>
...
<script lang="ts" setup>
  ...
  /** 流程分组 */
  let groups = ref<{ id: string; name: string }[]>([]);
  const loadGroups = () => {
  ...
  const loadIcons = () => {
    getIconSvgs().then((resp) => {
      icons.value = resp.data || [];
    });
  };

  const onIconChoosed = (name: string) => {
    (flowDef.value as any).icon = name;
  };
  ...
  // 当前的流程定义
  let flowDef = ref<FlowDefType>({});
  let rules = ref<Record<string, any>>({
  ...
  // 新增分组
  let group = ref<GroupType>({});
  let groupModel = ref();
  const handleOpenAddGroup = () => groupModel.value.show();
  const onSaveGroupCallback = (data: { id: string; name: string }) => {
    groups.value.push(data);
    (flowDef.value as any).groupId = data.id; // 选择新添加的分组
    formRef.value?.clearValidate?.('groupId');
  };
  ...
  onMounted(() => {
    (flowDef.value as any) = (flowDefinition as any).workFlowDef;
  });
