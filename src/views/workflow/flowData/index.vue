          ...
          >
            ...
            <template #content>
              ...
              <div class="search-more-box">
                ...
                <a-select
                  class="column"
                  v-model:model-value="query.groupId"
                  allow-clear
                  placeholder="流程分组"
                >
                  <a-option
          ...
          <template #columns>
            ...
            <a-table-column title="发起人" :width="120">
              <template #cell="{ record }">
                <flow-node-avatar
                  :size="20"
                  :id="record.initiatorId"
                ></flow-node-avatar
              ></template>
            </a-table-column>
            <a-table-column title="状态" :width="100">
              <template #cell="{ record }">
                <a-tag color="blue" v-if="record.status == STATUS.UNDERWAY">{{
                  STATUS_LIST[0].name
                }}</a-tag>
                <a-tag
                  color="green"
                  v-else-if="record.status == STATUS.APPROVED"
                  >{{ STATUS_LIST[1].name }}</a-tag
                >
                <a-tag
                  color="red"
                  v-else-if="record.status == STATUS.REJECTED"
                  >{{ STATUS_LIST[2].name }}</a-tag
                >
                <a-tag
                  color="orangered"
                  v-else-if="record.status == STATUS.CANCELLED"
      ...
      <div class="flow-transfer-box">
        ...
        >
          <a-form-item field="assigneeId" label="原审批人" :required="true">
            <a-select
              v-model:model-value="transferForm.assigneeId"
              allow-search
              placeholder="请选择原审批人"
            >
              <a-option
                v-for="user in flowAssignees"
                :value="user.id"
                :label="user.name"
              />
            </a-select>
          ...
          <a-form-item field="newAssigneeId" label="转办给" :required="true">
            <a-select
              v-model:model-value="transferForm.newAssigneeId"
              allow-search
              placeholder="请选择转办人"
            >
              <a-option
                v-for="user in flowOtherAssignees"
                :value="user.id"
                :label="user.name"
              />
            </a-select>
          ...
          <a-form-item field="comment" label="转办原因">
            <a-textarea
              v-model:model-value="transferForm.comment"
              placeholder="请输入转办原因"
              allow-clear
              :max-length="500"
              show-word-limit
...
<script lang="ts" setup>
  ...
  const query = ref({ current: 1, pageSize: 10, total: 0 });
  ...
  const onSearch = (current) => {
    if (current) query.value.current = current;
  ...
  const onSearchReset = () => {
    query.value = { current: 1, pageSize: 10, total: 0 };
  ...
  const onExport = () => {
    ...
    window.open(
      `${baseUrl}/flowdata/export?${new URLSearchParams(query.value)}`
    );
  };
  ...
  const onTransferOpen = (flowInst) => {
    ...
    FlowDataApi.listFlowInstAssignees({ flowInstId: flowInst.id }).then(
      (resp) => {
        if (resp.code == 1) {
          flowAssignees.value = resp.data || [];
          const flowAssigneeIds = flowAssignees.value.map((item) => item.id);
          flowOtherAssignees.value = users.value.filter(
            (item) => !flowAssigneeIds.includes(item.id)
          );
        }
      }
    );
