  ...
  <section class="flow-detail-container" v-loading.fullscreen="loading">
    ...
    <template v-else>
      <div class="flow-status-stamp"
        ><FlowStatusStamp :status="flowInst.status" :size="120"
      /></div>
      ...
      <div class="flow-detail-box">
        <div class="header-box">
          <div class="summary-info">
            ...
            <div class="status">
              <a-tag color="blue" v-if="flowInst.status == STATUS.UNDERWAY">{{
                STATUS_LIST[0].name
              }}</a-tag>
              <a-tag
                color="green"
                v-else-if="flowInst.status == STATUS.APPROVED"
                >{{ STATUS_LIST[1].name }}</a-tag
              >
              <a-tag
                color="red"
                v-else-if="flowInst.status == STATUS.REJECTED"
                >{{ STATUS_LIST[2].name }}</a-tag
              >
              <a-tag
                color="orangered"
                v-else-if="flowInst.status == STATUS.CANCELLED"
                >{{ STATUS_LIST[3].name }}</a-tag
              >
            </div>
          </div>
        ...
        <div class="flow-box">
          <a-timeline mode="left" labelPosition="relative">
              ...
              <a-timeline-item :label="formatUtcDateTime(node.auditTime)">
                <template #dot>
                  <div class="assignee-container">
                    ...
                    <template
                      v-else-if="[CMD.AUTO_APPROVED].includes(node.flowCmd)"
                    >
                      <a-avatar :size="36" style="background: #1989fa"
                        ><icon-robot
                      /></a-avatar>
                    ...
                    <template
                      v-else-if="[CMD.AUTO_REJECTED].includes(node.flowCmd)"
                    >
                      <a-avatar :size="36" style="background: #1989fa"
                        ><icon-robot
                      /></a-avatar>
                      ...
                      <template
                        v-else-if="
                          [
                            ...
                            CMD.AUTO_APPROVED,
                            CMD.AUTO_REJECTED,
                          ].includes(node.flowCmd)
                        "
                      >
                        审批
                      </template>
                      <template v-else-if="node.flowCmd == CMD.CANCELED"
                        >撤销</template
                      >
                      <template v-else-if="node.flowCmd == CMD.ASSIGN"
                        >转交</template
                      >
                      <template v-else-if="node.flowCmd == CMD.BACK"
                        >回退</template
                      >
                      <template v-else-if="node.flowCmd == CMD.ADD_BEFORE_SIGN"
                        >前加签</template
                      >
                      <template v-else-if="node.flowCmd == CMD.ADD_AFTER_SIGN"
                        >后加签</template
                      >
                      <template v-else-if="node.flowCmd == CMD.ADD_SIGN"
                        >并加签</template
                      >
                      <template v-else-if="node.flowCmd == CMD.DEL_SIGN"
                        >减签</template
                      >
                      <template v-else-if="node.flowCmd == CMD.COPY"
                        >抄送</template
                      >
                      <template v-else-if="node.flowCmd == CMD.COMMENT"
                        >评论</template
                      >
                      <template v-else-if="node.flowCmd == CMD.TRANSACT"
                        >办理</template
                      >
                        ...
                        <div class="node-sign-type">
                          <template v-if="node.multiInstanceApprovalType == 2">
                            <template v-if="node.type == NODE.APPROVE"
                              >只需一人审批同意</template
                            >
                            <template v-if="node.type == NODE.TRANSACT"
                              >只需一人办理同意</template
                            >
                          </template>
                          <template
                            v-else-if="
                              node.userIds.length + node.roleIds.length > 1
                            "
                          >
                            <template v-if="node.type == NODE.APPROVE"
                              >需所有人审批同意</template
                      ...
                      <template v-else>
                        ...
                        <template v-else>
                          <div class="assignee-name">{{
                            ArrayUtil.get(users, 'id', node.auditor).name
                          }}</div>
                          <div class="node-sign-assignee">
                            ...
                            <template
                              v-else-if="[CMD.DEL_SIGN].includes(node.flowCmd)"
                            >
                              减签
                              <div class="node-sign-assignee-name">{{
                                ArrayUtil.get(users, 'id', node.assignee).name
                            ...
                            <template
                              v-else-if="[CMD.ASSIGN].includes(node.flowCmd)"
                            >
                              转交给
                              <div class="node-sign-assignee-name">{{
                                ArrayUtil.get(users, 'id', node.assignee).name
                            ...
                            <template
                              v-else-if="[CMD.TRANSFER].includes(node.flowCmd)"
                            >
                              转办给
                              <div class="node-sign-assignee-name">{{
                                ArrayUtil.get(users, 'id', node.assignee).name
                      ...
                      <div class="comment-attachment" v-if="node.files">
                        ...
                        >
                          <div class="name"
                            ...
                            {{ attachment ? attachment.name : '' }}</div
                          >
            ...
            <template v-if="flowInst.status == STATUS.UNDERWAY">
              <a-timeline-item
                v-for="node in flowFutureNodes.filter(
                  (node) => node.nodeType != NODE.END
                )"
                class="future-node-box"
              >
                <template #dot>
                  <div class="assignee-container">
                ...
                <div class="audit-record">
                  <div class="avatar sgement">
                    ...
                    <div class="auditor-name">
                      <div class="node-sign-type">
                        <template v-if="node.multiInstanceApprovalType == 2">
                          <template v-if="node.nodeType == NODE.APPROVE"
                            >只需一人审批同意</template
                          >
                          <template v-if="node.nodeType == NODE.TRANSACT"
                            >只需一人办理同意</template
                          >
                        </template>
                      ...
                      <div
                        v-if="node.userIds && node.userIds.length"
                        class="node-assignee"
                      >
                        <flow-node-avatar
                          v-for="userId in node.userIds"
        ...
        <a-dropdown
          ...
          v-if="
            ...
            (flowInst.assignable ||
              flowInst.backable ||
              (flowInst.signable &&
                [NODE_SIGN.NONE].includes(flowInst.nodeSignType)))
          "
      ...
      <div class="flow-exe-box">
        <a-form :model="handleModalForm" layout="vertical">
          <template v-if="handleModalForm.flowCmd == CMD.ASSIGN">
            ...
            >
              <a-select
                v-model:model-value="handleModalForm.assignee"
                allow-search
                placeholder="请选择转交人员"
              >
                <a-option
          ...
          <template
            v-if="
              [CMD.ADD_BEFORE_SIGN, CMD.ADD_AFTER_SIGN, CMD.ADD_SIGN].includes(
          ...
          >
            <a-form-item
              ...
              :required="
                [
                  ...
                  CMD.ADD_AFTER_SIGN,
                  CMD.ADD_SIGN,
                ].includes(handleModalForm.flowCmd)
              "
            >
              <a-radio-group
                v-model:model-value="handleModalForm.flowCmd"
                class="modify-sign-group"
              >
                <a-radio :value="CMD.ADD_BEFORE_SIGN">
                  前加签
                  <a-tooltip
                    content="在当前节点之前增加一个审批节点，当新增的节点同意后，再流转至当前节点。"
                    mini
                  >
                ...
                <a-radio :value="CMD.ADD_AFTER_SIGN">
                  后加签
                  <a-tooltip
                    content="在当前节点之后增加一个审批节点，当前节点会默认同意，并流转至新增的节点。"
                    mini
                  >
                    <icon-question-circle class="modify-sign-tip" />
                  </a-tooltip>
            ...
            >
              <a-select
                v-model:model-value="handleModalForm.userId"
                allow-search
                placeholder="请选择加签人员"
              >
                <a-option
          ...
          >
            ...
            >
              <a-option
                v-for="user in users.filter((i) =>
                  deleteableUserIds.includes(i.id)
                )"
                :value="user.id"
                :label="user.name"
              />
          ...
          <template v-if="handleModalForm.flowCmd == CMD.BACK">
            ...
            >
              <a-select
                v-model:model-value="handleModalForm.flowNodeId"
                allow-search
                placeholder="请选择回退节点"
              >
                <a-option
          ...
          >
            <a-textarea
              ...
              :placeholder="`请输入${handleModalCommentLabel(
                handleModalForm.flowCmd
              )}`"
              allow-clear
              :max-length="128"
          ...
          <!-- 签名 -->
          <template
            v-if="handleModalForm.flowCmd == CMD.APPROVED && flowInst.signature"
          >
            <a-form-item
              field="signature"
              label="签名"
              :required="flowInst.signature"
            >
              <div class="action-signature-box">
                <SignaturePad
                  width="480px"
                  height="200px"
                  @ok="handleSignatureOk"
                  @clear="handleSignatureCancel"
                />
              </div>
            </a-form-item>
          </template>
...
<script lang="ts" setup>
  ...
  let fileUploadHeaders = ref({ Authorization: `Bearer ${getToken()}` }); // 文件上传请求头
  ...
  const loadFlowDetail = () => {
    getFlowInstanceDetails(props.flowInst.id).then((resp) => {
      let {
        nodes,
        futureNodes,
      ...
      //表单
      formWidgets.value = _formWidgets;
      formWidgetMap.value = formWidgetListToMap(_formWidgets);
      formValue.value = JSON.parse(_formValue);
      formattedFormValue.value = JSON.parse(_formValue);
    });
  };
  ...
  const handleFileListChange = (fileList, fileItem) => {
    ...
    if (resp.code === undefined || resp.code == 1 || resp.code == 10000) {
      ...
      // 上传失败提示，删除文件
      Message.error(data.message || data.msg || '文件上传失败');
      ArrayUtil.remove(fileList, 'uid', fileItem.uid);
    }
  };
  ...
  const onHandleModalOK = () => {
    ...
    if (fileIds && fileIds.length > 0)
      handleModalFormValue.fileIds = fileIds.map((v) => v.id);

    let req = null;
    loading.value = true; // 开启遮罩
    switch (cmd) {
      case CMD.CANCELED:
        req = cancel(handleModalFormValue);
        break;
      case CMD.COMMENT:
        req = comment(handleModalFormValue);
        break;
      case CMD.BACK:
        req = jump(handleModalFormValue);
        break;
      case CMD.ADD_BEFORE_SIGN:
        req = addSign(handleModalFormValue);
        break;
      case CMD.ADD_AFTER_SIGN:
        req = addSign(handleModalFormValue);
        break;
      case CMD.ADD_SIGN:
        ...
        break;
      case CMD.DEL_SIGN:
        req = delSign(handleModalFormValue);
        break;
      case CMD.APPROVED:
  ...
  // 再次申请
  const onRelaunch = () => {
    FlowManApi.checkRelaunchable({ flowDefId: props.flowInst.flowDefId }).then(
      (resp) => {
