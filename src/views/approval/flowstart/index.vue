      ...
      <template v-else v-for="group in filteredFlowGroups">
        ...
        >
          ...
          <div class="group-body">
            <a-card
              hoverable
              v-for="flow in group.flowDefinitions"
              class="flow-item-box"
...
// @ts-nocheck
  ...
  import { getEnabledFlowGroupWithDef, getFlowFormWidget } from '@/api/server/workflow/flow';
  ...
  let filteredFlowGroups = computed(() => {
    return ObjectUtil.copy(groups.value).filter((group) => {
      ...
      group.flowDefinitions = (flows || []).filter((flow) =>
        flow.name.includes(flowName.value)
      );
  ...
  const fetchFlowFormWidget = (flowDefId) => {
    getFlowFormWidget(flowDefId).then((resp) => {
...
<style lang="less" scoped>
  ...
  .flow-start-box {
    ...
    .flow-group-wrapper {
      height: calc(
        100vh - @AppHeaderHeight - @AppBreadcrumbHeight - @SearchBoxHeigth -
          @LayoutGap
      );
