...
<script lang="ts" setup>
  ...
  import { queryMyApplyFlowInstances } from '@/api/server/workflow/flow';

  let groups = ref([]);

  let query = ref({ current: 0, pageSize: 10, total: 0 });
  let flowInsts = ref([]);
  ...
  const onSearchReset = () => {
    query.value = { current: 0, pageSize: 10, total: 0 };
  ...
  const loadFlowInsts = () => {
    query.value.current++;
    queryMyApplyFlowInstances(query.value).then((resp) => {
