...
<template>
  <div class="flow-form-container">
      ...
      <template v-for="widget in flowWidgets">
        ...
        >
          ...
          <template v-else-if="widget.type == WIDGET.RICH_TEXT">
            <RichText
              v-model:value="flowForm[widget.name]"
              :placeholder="widget.placeholder"
            />
          </template>
          ...
          <template v-else-if="widget.type == WIDGET.SINGLE_CHOICE">
            <a-select
              ...
              :allow-clear="!widget.required"
            >
              <a-option v-for="option in widget.options" :value="option">{{
                option
              }}</a-option>
            </a-select>
          </template>
          ...
          <template v-else-if="widget.type == WIDGET.DEPARTMENT">
            <a-select
              ...
              :allow-clear="!widget.required"
            >
              <a-option v-for="dept in allDepts || []" :value="dept.id">{{
                dept.name
              }}</a-option>
            </a-select>
          </template>
          <template v-else-if="widget.type == WIDGET.EMPLOYEE">
            <a-select
              ...
              :allow-clear="!widget.required"
            >
              <a-option v-for="user in allUsers || []" :value="user.id">{{
                user.name
              }}</a-option>
            </a-select>
          </template>
          ...
          <template v-else-if="widget.type == WIDGET.FLOW_INST">
            <div class="flow-inst-widget">
              ...
              <div class="flow-inst-list">
                <FlowCard
                  v-for="id in flowForm[widget.name]"
                  :flow-inst-id="id"
                ></FlowCard>
              </div>
              <FlowInstSelect
                ...
                v-model:selected="flowForm[selectedFlowInstWidget.name]"
              ></FlowInstSelect>
            </div>
          </template>
        ...
        <template v-else-if="widget.type == WIDGET.DESCRIBE">
          <div class="describe"
            ><icon-info-circle />{{ widget.placeholder }}</div
          >
        </template>
        <template v-else-if="widget.type == WIDGET.DETAIL">
          <FlowLaunchWidgetDetail
            :widget="widget"
            :headers="fileUploadHeaders"
            :url="FILE_UPLOAD_URL"
            :form="flowForm"
          ...
          <template v-for="node in flowNodes">
            ...
            >
              ...
              <div class="node-box" v-else-if="node.nodeType == NODE.APPROVE">
                <div class="node-name">
                  {{ node.name }}
                  <div class="node-type" v-if="node.approvalType == 1"
                    >自动通过</div
                  >
                  <div class="node-type" v-if="node.approvalType == 2"
                    >自动拒绝</div
                  >
                  <div
                    class="node-type"
                    v-if="node.multiInstanceApprovalType == 1"
                    >需所有人审批同意</div
                  >
                  <div
                    class="node-type"
                    v-if="node.multiInstanceApprovalType == 2"
                    >只需一人审批同意</div
                  >
                  <div
                    class="node-type"
                    v-if="node.multiInstanceApprovalType == 3"
                    >需依次审批同意</div
                  >
                </div>
                <div class="node-content">
                  <div v-if="node.initatorChoice" class="initator-chioce">
                    <a-button
                      size="small"
                      @click="onChooseUserClicked(node.id)"
                    >
                  ...
                  <template v-else>
                    <div
                      ...
                      v-if="
                        node.approvalType == 0 &&
                        node.userIds.length == 0 &&
                        node.roleIds.length == 0
                      "
                    >
    ...
    <a-alert v-else type="warning"
      >必填信息填写完整后，将显示审批流程，之后方可提交审批。</a-alert
    >
    <!-- 按钮 -->
    ...
    <section class="flow-btn-box">
      ...
      <a-button
        type="primary"
        @click="handleOk()"
        :disabled="!formValidated || launching"
        >提交</a-button
      >
    </section>
...
<script lang="ts" setup>
  ...
  import { launchFlow, viewProcessChart } from '@/api/server/workflow/flow';
  ...
  const formWidgetsToMap = () => {
    flowWidgetMap = formWidgetListToMap(props.flowWidgets || []);
  };
  ...
  const validFlowForm = () => {
    ...
    (props.flowWidgets || []).forEach((flowWidget) => {
      ...
      if (required) {
        ...
        detailWidgets = detailWidgets.filter(
          (detailWidget) => detailWidget.required
        );
  ...
  const handleFormValue = (flowWidgets, flowValue) => {
    // 组件map是否已经初始化
    if (!!!flowWidgetMap) {
      flowWidgetMap = formWidgetListToMap(flowWidgets);
    }
    // 处理表单值
      ...
      if ([WIDGET.PICTURE, WIDGET.ATTACHMENT].includes(widget.type)) {
        ...
        flowValue[name].forEach((flowDetailValue) => {
          for (let detailName in flowDetailValue) {
            ...
            if (
              [WIDGET.PICTURE, WIDGET.ATTACHMENT].includes(detailWidget.type)
            ) {
              flowDetailValue[detailName] = (
                flowDetailValue[detailName] || []
              ).map((v) => v.id);
            }
          }
  ...
  const handleOk = () => {
    ...
    if (formValidated.value) {
      launchFlow({
        ...
        flowValue: JSON.stringify(
          handleFormValue(props.flowWidgets, ObjectUtil.copy(flowForm.value))
        ),
  ...
  const formAutoFill = () => {
    ...
    widgets.forEach((widget) => {
      ...
      if (WIDGET.DETAIL == type) {
        ...
        (details || []).forEach(
          (detailWidget) => (detailValue[detailWidget.name] = null)
        );
      }
    });
  };
      ...
      if ([WIDGET.NUMBER, WIDGET.MONEY].includes(type)) {
        ...
        if (ids && ids.length > 0) {
          FileApi.batchMetadata({ ids: ids.join(',') }).then((resp) => {
            ...
            attachments.forEach(
              (i) => (i.url = `${FILE_DOWNLOAD_URL}?id=${i.id}`)
            );
